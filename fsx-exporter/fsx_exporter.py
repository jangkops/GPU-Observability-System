...
# HELP fsx_user_quota_bytes FSx user quota usage in bytes
# TYPE fsx_user_quota_bytes gauge
'''
                
                # 사용자별 quota (캐시 파일에서 읽기)
                import json
                try:
                    with open('/var/cache/fsx-quota.json') as qf:
                        for item in json.load(qf):
                            metrics += 'fsx_user_quota_bytes{{username="{}"}} {}\n'.format(item["username"], item["bytes"])
                except:
                    pass
